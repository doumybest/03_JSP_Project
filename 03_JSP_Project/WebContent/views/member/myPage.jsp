...
<body>
	...
	<!-- Session에 담긴 USER정보 가져오기 -->
	<%
		String userId = loginUser.getUserId();
		String userName = loginUser.getUserName();
		
		// 아래는 필수 사항이 아님
		// value값들에 null값이 들어갈 수 있으므로 ""로 대채
	...
	<div class="outer">
		...
		<form id="mypage-form" action="<%= contextPath %>/update.me"
			...
			<table>
				<tr>
					<td>* 아이디</td>
					<td><input type="text" name="userId" maxlength="12" readonly
						value="<%= userId %>"></td>
					<!-- 수정을 막기 위해 readonly 속성 부여 -->
					<td></td>
				</tr>
				<tr>
					<td>* 이름</td>
				...
				<tr>
					...
					<td colspan="2">
						...
						type="checkbox" name="interest" id="sports" value="운동"
						<%= interest.indexOf("운동") > -1 ? "checked" : "" %>> <label
						for="sports">운동</label> <input type="checkbox" name="interest"
						id="hiking" value="등산"
						<%= interest.indexOf("등산") > -1 ? "checked" : "" %>> <label
						for="hiking">등산</label> <input type="checkbox" name="interest"
						id="fishing" value="낚시"
						<%= interest.indexOf("낚시") > -1 ? "checked" : "" %>> <label
						for="fishing">낚시</label> <br> <input type="checkbox"
						name="interest" id="cooking" value="요리"
						<%= interest.indexOf("요리") > -1 ? "checked" : "" %>> <label
						for="cooking">요리</label> <input type="checkbox" name="interest"
						id="game" value="게임"
						<%= interest.indexOf("게임") > -1 ? "checked" : "" %>> <label
						for="game">게임</label> <input type="checkbox" name="interest"
						id="movie" value="영화"
						<%= interest.indexOf("영화") > -1 ? "checked" : "" %>> <label
						for="movie">영화</label>
					</td>
				</tr>
			</table>
			...
			<script>
				...
				$(function() {
					...
					$("input[type=checkbox][name=interest]").each( function() {
						...
						// interest문자열로부터 현재 체크박스의 value가 포함되어 있지 않다면 -1반환
